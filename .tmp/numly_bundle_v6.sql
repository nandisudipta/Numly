...
DROP FUNCTION IF EXISTS get_user_role_in_business(uuid, uuid) CASCADE;
...
DROP FUNCTION IF EXISTS is_captain_or_vice(uuid, uuid) CASCADE;
...
DROP FUNCTION IF EXISTS set_business_invite_code() CASCADE;
...
DROP FUNCTION IF EXISTS join_business_with_code(text) CASCADE;
...
DROP FUNCTION IF EXISTS is_business_member(uuid, uuid, text[]) CASCADE;
...
DROP FUNCTION IF EXISTS accept_business_invitation(uuid) CASCADE;
...
DROP FUNCTION IF EXISTS get_user_businesses_with_roles() CASCADE;
...
DROP FUNCTION IF EXISTS has_book_access(uuid, uuid) CASCADE;
...
DROP FUNCTION IF EXISTS has_ledger_access(uuid, uuid) CASCADE;
...
-- Forces a known-good final state regardless of what earlier migrations
-- left behind (some "fix" migrations dropped policies by the wrong name,
-- so recursive ones survived).

-- 1. business_members: nuke all policies, recreate canonical non-recursive ones
DO $$
DECLARE r record;
BEGIN
...
-- 2. businesses: nuke + canonical owner-based policies
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT policyname FROM pg_policies
...
-- 3. Re-grant table privileges (in case anything was revoked)
...
-- 4. activity_log FK: PostgREST needs an FK to public.profiles for embed
...
-- 5. Make sure handle_new_user trigger backfills the test user(s) that
--    signed up before the trigger existed. Ensures every auth.users has profile.
